...
const get_nearest = function(resultA, resultB) {
    ...
    (
        ...
        (
            resultA.found && resultB.found &&
            resultA.distance < resultB.distance
        )
    )
    {
...
const return_simple_velocity = function(final_result, times, start, cb) {
    ...
    if("point_result" in final_result && "object_result" in final_result)
    {
...
rmns.NEAREST_VPOINT_OK = function(nearest, distance) {
...
rmns.remote_velocity = function(data, cb) {
    ...
    const return_velocity = json.ignoreHeuristic ?
